...
export function BadGateway({
  description = "A conexão com nossa cozinha foi interrompida. Estamos trabalhando para restabelecer o serviço.",
}: {
  description?: string
}) {
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-muted/30 to-background flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-2xl mx-auto text-center space-y-8">
        ...
        <div className="flex justify-center mb-8">
          <div className="float-animation">
            <SkewerIcon className="w-24 h-24 drop-shadow-lg opacity-50" />
          </div>
        </div>

        {/* 502 with flame icons */}
        <div className="space-y-4">
          <div className="flex items-center justify-center gap-4 text-6xl md:text-8xl font-bold text-chart-2">
            <FlameIcon className="w-12 h-12 md:w-16 md:h-16 glow-animation text-chart-2" />
            <span className="tracking-tighter">502</span>
            <FlameIcon
              className="w-12 h-12 md:w-16 md:h-16 glow-animation text-chart-2  [animation-delay:1s]"
            />
          </div>

          <div className="flex items-center justify-center gap-3 text-chart-2/80">
            <GrillIcon className="w-5 h-5" />
            <h2 className="text-lg md:text-xl font-semibold tracking-tight">Gateway com problemas</h2>
            <GrillIcon className="w-5 h-5" />
          </div>
        </div>
        ...
        <div className="space-y-3">
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-foreground text-balance">
            Conexão com a cozinha interrompida
          </h1>
        ...
        <p className="text-sm text-muted-foreground/70 pt-8">Aguarde um momento enquanto religamos o fogo!</p>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 flex justify-center gap-8 pb-8 pointer-events-none opacity-20">
        <FlameIcon className="w-8 h-8 glow-animation text-chart-2" />
        <FlameIcon className="w-6 h-6 glow-animation text-chart-2 [animation-delay:0.5s]" />
        <FlameIcon className="w-10 h-10 glow-animation text-chart-2 [animation-delay:1.5s]" />
        <FlameIcon className="w-7 h-7 glow-animation text-chart-2 [animation-delay:2s]" />
      </div>
    </div>
  )
}
